CREATE TABLE user_intents_with_attachments (
  intent_id TEXT PRIMARY KEY CHECK (length(intent_id) > 0),
  session_id TEXT NOT NULL REFERENCES sessions(session_id) ON DELETE RESTRICT,
  text TEXT,
  attachments_json TEXT NOT NULL DEFAULT '[]' CHECK (json_valid(attachments_json) AND json_type(attachments_json) = 'array'),
  ...
  CHECK (
    (status = 'pending'
      AND text IS NOT NULL AND (length(trim(text)) > 0 OR json_array_length(attachments_json) > 0)
...
INSERT INTO user_intents_with_attachments(intent_id,session_id,text,content_digest,delivery_mode,status,queue_sequence,queued_behind_turn_id,target_turn_id,created_at,updated_at,held_at,promoted_at,delivered_at,unresolved_at,withdrawn_at,steer_origin,attempt_count) SELECT intent_id,session_id,text,content_digest,delivery_mode,status,queue_sequence,queued_behind_turn_id,target_turn_id,created_at,updated_at,held_at,promoted_at,delivered_at,unresolved_at,withdrawn_at,steer_origin,attempt_count FROM user_intents;
DROP TABLE user_intents;
ALTER TABLE user_intents_with_attachments RENAME TO user_intents;
...
CREATE TRIGGER user_intent_attachments_delivery_only
BEFORE UPDATE OF attachments_json ON user_intents
WHEN NOT (OLD.attachments_json IS NEW.attachments_json OR
 (NEW.attachments_json = '[]' AND OLD.status IN ('dispatching', 'unresolved') AND NEW.status = 'delivered'))
BEGIN SELECT RAISE(ABORT, 'User intent attachments may only be cleared by delivery'); END;
