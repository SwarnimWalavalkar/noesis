...
`files.read` may read an explicitly named file anywhere the Noesis process can read. This does not widen file writes, directory traversal, or search. Those remain bound to their declared project resources. `files.write` is the single file-mutation primitive. Write mode creates or completely replaces one UTF-8 file and creates parent directories by default. Replace mode validates one or more exact, non-overlapping edits against the same original file, then commits the result once. A missing, ambiguous, no-op, or overlapping replacement fails without changing the file. Compact skill metadata names a skill without presenting its storage path as a project file. The model loads the frozen body through `execute` and `tools.skills.load({ name })`. Ordinary instruction files are still readable when explicitly named.

The Broker does not impose a generic byte ceiling on a valid tool result. Each tool definition or MCP server owns the shape and context sensitivity of its result. Operational result records remain authoritative. At the provider boundary, completed direct-tool and `execute` results above 32 KiB receive a bounded initial head/tail presentation. Production saves the exact serialized returned result through WorkspaceStore and includes its digest, byte count, and recovery path in the preview. Upstream log truncation remains explicit; the artifact cannot recover bytes already discarded by a tool. This presentation is fixed when ingested instead of rewriting it on every request.

A missing artifact writer or failed presentation save still returns a bounded preview and preserves the completed tool-call outcome. It explicitly marks exact recovery unavailable and tells the model not to repeat the completed call to recover output. It never claims a recovery path that was not successfully saved. Omitted content remains unknown. The turn context allocator may also project older results when constructing a later request.
...
Foreground `execute` has REPL-style completion: its final top-level expression or explicit `return` supplies the `value` in the model-visible result. Captured `stdout` and `stderr` appear alongside that value; `logsTruncated` marks logs that exceeded the existing retention allowance. Each call has a fresh async function scope with normal local shadowing of SDK names. `store` and `load` explicitly preserve scratch values across calls. `Object.keys(tools)` and `Object.keys(tools.family)` enumerate names from the same frozen Broker catalog. Saved Program source retains ordinary async function-body semantics and must return its schema-valid output explicitly.
...
Inside `execute`, `agents.spawn({ name?, systemPrompt?, prompt, tools?, thinkingLevel? })` durably admits a retained process-scoped actor and returns stable agent and task handles immediately. The prompt may be text, a lazy `ContextView`, or an array of either; the host expands context views only after checking that they belong to the active execution's frozen document. `agents.send`, `list`, `inspect`, `wait`, `cancel`, and `close` provide one asynchronous collaboration surface for foreground-to-child and child-to-child work. Waiting joins an exact task but never owns or cancels it.

The parent may choose the child system prompt, prompt, thinking level, and canonical Code Mode tools from the turn's frozen Tool Catalog. It cannot choose provider or model. The default child route is user-configurable and frozen at admission; the route seam remains explicit for a future policy that may safely admit per-run choices. A child receives the same fixed provider-facing surface as the foreground: file read, file write with exact replacement, shell, and `execute`. Programs and all collaboration operations remain Broker tools available only inside Code Mode. Recursive spawning is possible only when `agents.spawn` was explicitly granted.

A retained agent outlives the foreground turn and session that created it, but not the primary Noesis process. It has no independent provider-round, tool-call, or wall-clock ceiling; explicit cancel, close, or process shutdown are its lifecycle boundaries. Every provider round and child tool effect reserves and settles under the child principal using its frozen authority. SQLite records actors, tasks, mailbox delivery, exact timeline positions, model calls, artifacts, usage, errors, and nested Code Mode lineage. The main transcript contains only foreground activity. The fixed TUI surface shows active process actors across sessions, and `Ctrl+O` exposes completed actors plus their prompt, reasoning, assistant stream, model rounds, nested tools, messages, route, usage, and errors.
