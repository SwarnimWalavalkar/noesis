...
Codemode checks explicit completeness fields before semantic synthesis. Oversized `shell.run` output remains available at `fullOutputPath`; codemode inspects the complete artifact with `files.read` line ranges or ordinary Unix tools rather than repeating the command. When other required evidence reports `truncated: true`, it narrows or recollects that evidence. Omitted output is unavailable evidence, not evidence that the omitted item does not exist.
