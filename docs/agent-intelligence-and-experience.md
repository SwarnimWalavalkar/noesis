...
Use one direct tool for a simple operation. For multi-call work, plan collection and synthesis before one coherent execute program, batch independent calls, keep intermediate results in code, and use agents.run when a bounded independent agent can help. If that program reveals a specific evidence gap, use one coherent follow-up instead of a series of direct calls.
