...
Bare `/reasoning` opens a picker and `/reasoning LEVEL` changes effort inline for the current session. `/model` opens a fuzzy-searchable picker for the current provider, while `/model ID` offers live current-provider model completions as a fast path. `/provider` first chooses a provider and then its model. A route picker opens immediately from Pi's bundled and persisted catalog, refreshes compatible model metadata from `pi.dev` in the background, and retains its existing choices if refresh fails. The route pickers finish by choosing one of the model's supported reasoning levels. Before creating the new session, Noesis checks the selected provider's existing Pi authentication. A missing subscription credential continues directly into that provider's OAuth flow; an API-key provider opens a dedicated masked input. Authentication input remains ephemeral UI state and never becomes a transcript message or model input. Cancelling authentication preserves the current session. Selecting an authenticated different provider or model starts a fresh session, preserves the previous session for later resume, and never replays its transcript through the new route. Catalog refresh affects only later selections; it does not rewrite the active route, prompt, transcript, or tool schema. `/resume` opens the saved-session picker and restores the selected session through the same resumability path used at startup. The picker can delete an idle, non-current session from resumable and searchable history after confirmation; exact learning and audit evidence remains intact. Sessions with no conversation or recoverable queued work are purged instead of becoming saved-session clutter. Selecting the current model with a different reasoning level updates only the current session. The picker and the settled TUI say which case applies so prompt-cache isolation is a visible product guarantee, not an implementation surprise.
