...
Every active retained agent appears in a bounded process-wide surface fixed above the composer, so its status does not scroll away when the user changes sessions. Completed actors stay collapsed until `Ctrl+O` reveals them. Arrow keys select actor rows, Tab moves between actor and foreground transcript navigation, and Enter opens the complete inspector. The inspector streams and retains the child prompt, reasoning, assistant text, model rounds, nested tool and Code Mode activity, collaboration messages, usage, and errors. Child internals never enter the foreground transcript.
