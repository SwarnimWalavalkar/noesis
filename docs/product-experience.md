...
`/context` opens a compact inspector with one input-budget bar, estimated usage, and a token count for each section. Empty sections start collapsed. Enter opens a section's content preview. Tools appear as separate names and descriptions; `s` shows or hides their indented parameter schemas. The Skill catalog shows highlighted names and readable descriptions, with XML escapes decoded. Truncated skill catalogs still format complete entries and preserve unfinished or unrecognized material as captured text. Truncated or unrecognized tool previews remain visible as captured text. The `?` view shows the model window, input budget, output reserve, and accounting details.
...
After a request, the inspector shows the last request captured after tool-result projection. Before a request is available in this process, it shows a labelled startup or resumed-session preview. This preview includes the base system prompt, skill metadata, the actual four foreground tool definitions, and retained conversation. Preview and execution share the tool contracts and Pi adapters, so inspection needs no model call or executable tool handlers. Capability selection resolves when a turn is admitted. Installed skill bodies do not count until loaded. Component estimates exclude provider-specific framing and do not claim to reproduce billing.
...
Explicit steering stays inside the active turn and reaches the model at the earliest safe boundary. Until delivery, the queue surface shows the exact steering message as waiting for that boundary. A tool call that has already started finishes normally. If the same assistant response proposed more tool calls, Noesis records the not-yet-started calls as skipped and delivers all pending steering messages before the next model request. An active `execute` call remains one atomic tool call, including its nested work.

The main chat renders provider-visible thinking as a distinct, subdued trace before the corresponding answer. Thinking remains inspectable after resume but never becomes conversation history or a model instruction. While a turn is active, one fixed-width status glyph animates without moving the surrounding footer; it stops as soon as the work settles.

Quitting disables new input and shows an animated closing state through application cleanup, before relinquishing the terminal. Pi sessions release provider-owned connections and timers when they close, including when the installed package contains separate Pi dependency instances.

Bare `/reasoning` opens a picker and `/reasoning LEVEL` changes effort inline for the current session. `/model` opens a fuzzy-searchable picker for the current provider, while `/model ID` offers live current-provider model completions as a fast path. The model picker finishes by choosing one of the model's supported reasoning levels. `/provider` instead opens a connection-focused provider picker: it shows connection sources, selects a provider without opening the model list, and can disconnect stored OAuth or API-key credentials after confirmation. Environment-owned credentials are labelled and remain externally managed. Selecting a disconnected provider opens its OAuth or masked API-key flow first; only successful authentication allows catalog refresh and creation of a new session on the provider's default model. Authentication input remains ephemeral UI state and never becomes a transcript message or model input. Cancelling authentication preserves the current session. Selecting an authenticated different provider or model starts a fresh session, preserves the previous session for later resume, and never replays its transcript through the new route. The current model catalog renders immediately, refreshes compatible metadata from `pi.dev` in the background, and retains existing choices if refresh fails. Catalog refresh affects only later selections; it does not rewrite the active route, prompt, transcript, or tool schema. If a model request receives HTTP 401 before visible output while using stored OAuth, Noesis refreshes the credential atomically and retries once across foreground, background-role, subagent, and MCP sampling calls. It never replays a request after visible output; repeated rejection, refresh failure, and API-key rejection all return reconnect guidance. `/resume` opens the saved-session picker and restores the selected session through the same resumability path used at startup. The picker can delete an idle, non-current session from resumable and searchable history after confirmation; exact learning and audit evidence remains intact. Sessions with no conversation or recoverable queued work are purged instead of becoming saved-session clutter. Selecting the current model with a different reasoning level updates only the current session. The picker and the settled TUI say which case applies so prompt-cache isolation is a visible product guarantee, not an implementation surprise.
...
The user asks Noesis to investigate a repository, change files, run commands, search the web, or create an artifact. One simple file or shell operation can use a direct tool. Work that requires multiple tool calls or transforms intermediate results normally runs as one coherent JavaScript program through `execute`, rather than one wrapper for each known call. The program plans collection and synthesis together. It checks explicit completeness fields before synthesis: `shell.run` saves oversized output to an ordinary artifact file and returns its path without terminating the command at the artifact storage boundary. A non-truncated in-memory `output` is complete. For a truncated preview, the program inspects a complete artifact with bounded reads or ordinary Unix tools, and narrows or recollects evidence when that artifact is incomplete. Other required evidence marked `truncated: true` is handled the same way, and omitted output is never treated as proof of absence. Session search already combines lexical and semantic retrieval with reranking, so one precise query normally suffices. It opens its strongest evidence before returning and reports an empty or irrelevant result only as a bounded search that found no relevant evidence instead of searching through paraphrases. The progressively loaded `execute` skill carries the turn-frozen Code Mode and SDK guidance; exact tools remain discoverable through the frozen catalog.

Foreground `execute` behaves like a small JavaScript REPL: a final top-level expression is returned automatically, while explicit `return` remains supported. Reusable Program bodies keep explicit function semantics so their typed output stays deliberate.

The conversation gets the final result. The user can still inspect each tool call and revision.

For analysis that benefits from more history than the foreground prompt should carry, `execute` can inspect the complete pre-turn session as a lazy immutable document and pass one selected slice to `agents.spawn`. Spawning returns immediately, so the foreground can continue working, send later corrections, start another session, or join the exact task only when its result becomes necessary. Retained agents may also message one another through the same durable mailbox. This keeps the ordinary prompt bounded without making a synchronous model query the unit of delegation.

The subagent panel appears above the composer only when the selected session has spawned a subagent. Historical records alone do not show the panel or capture `Ctrl+O` navigation in a new session. Once visible, the panel includes retained agents from earlier sessions. Completed actors stay collapsed until `Ctrl+O` reveals them. Resuming a session that spawned agents restores the panel. Switching sessions changes visibility without cancelling agents or deleting their records. Arrow keys select actor rows, Tab moves between actor and foreground transcript navigation, and Enter opens the complete inspector. The inspector streams and retains the child prompt, reasoning, assistant text, model rounds, nested tool and Code Mode activity, collaboration messages, usage, and errors. Child internals never enter the foreground transcript.
