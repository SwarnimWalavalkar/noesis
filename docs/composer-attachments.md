...
Models with image support receive current and retained-history images without Noesis image-count, per-image byte, aggregate-byte, or dimension caps. Thumbnail decoder bounds do not restrict model input. Noesis does not gate images on a provider-neutral token estimate: image costs depend on the provider's processing rules, and reported usage informs subsequent context accounting. Actual provider format, payload, and context restrictions still apply and may reject a request. Image payloads must fit available process memory because the provider SDK requires base64 blocks; originals remain streamed, durable files regardless. Unsupported-model notices preserve original file references and only appear for the current message.
