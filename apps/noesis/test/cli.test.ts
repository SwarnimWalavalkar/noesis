...
async function runCli(
  ...
  const code = await new Promise<number | null>((resolveExit, reject) => {
    const timeout = setTimeout(() => {
      child.kill("SIGKILL");
      reject(new Error(`CLI did not exit within 30 seconds. Output:\n${output}`));
    }, 30_000);
...
describe("Noesis CLI grammar", () => {
  test("reports malformed arguments with a failing exit status", async () => {
    const result = await runCli(["inspect", "--continue"]);
    expect(result.code).toBe(1);
    expect(result.output).toContain("--continue is available only with the tui command");
  });
